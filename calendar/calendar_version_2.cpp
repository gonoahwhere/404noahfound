...
bool leapYearChecker(int year) { return (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0); } /* Creates a True/False statement to check if year given is divisible by 4 or 400, but not by 100 (centuries aren't leap years!) */
...
bool validateChecker(int &day, int &month, int &year) { /* Checks if the user provides a valid month, year or day, if not allows a retry */
	...
	do { cout << "Enter a Month [1-12]: "; cin >> month; if (month < 1 || month > 12) cout << "Invalid month, please enter 1-12!\n"; } /* Prints a string to the console and requires user input */
	while (month < 1 || month > 12); /* Lets user try again until valid value */
	do { cout << "Enter a Year [1970+]: "; cin >> year; if (year < 1970) cout << "Invalid year, please enter a year from 1970 onwards!\n"; }
	while (year < 1970);
	if (leapYearChecker(year)) daysInMonth[1] = 29;
	do { cout << "Enter a Day [1-" << daysInMonth[month - 1] << "]: "; cin >> day; if (day < 1 || day > daysInMonth[month - 1]) cout << "Invalid day, please enter 1-" << daysInMonth[month - 1] << "!\n"; }
	while (day < 1 || day > daysInMonth[month - 1]); cout << "\n\n";
	int startDay = generateCorrectDay(month, year);
	generateCalendar(month, year, startDay, day);
	cout << "\n\n"; return 0; /* Allows 'Press any key to continue... to be on a new line instead of next to the days */
}
