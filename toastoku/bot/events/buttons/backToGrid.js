...
const { titleCase, createGridButtons, getTurnEmbedFields, getConflictingCells, drawSudokuGrid, isPuzzleInSolutions } = require('../../commands/global/sudoku');
...
module.exports = async function handleBackToGrid(interaction) {
    if (interaction.customId !== 'back_to_grid') return;
    ...
    await SudokuGame.updateOne( { gameId }, { puzzle: gameState.puzzle, selectedGrid: null, selectedCell: null, selectedValue: null, conflictSet: [...gameState.conflictSet], lastUpdated: Date.now(), });
    ...
    const buttonRows = createGridButtons(gameState, gameState.mode === 'multi' ? gameState.joinedPlayers[gameState.currentTurnIndex] : interaction.user.id);
