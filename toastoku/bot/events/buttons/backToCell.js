...
const { titleCase, getTurnEmbedFields, createCellButtons, getConflictingCells, drawSudokuGrid, isPuzzleInSolutions } = require('../../commands/global/sudoku');
...
module.exports = async function handleBackToCell(interaction) {
    if (interaction.customId !== 'back_to_cell') return;
    ...
    if (!game) {
        return interaction.reply({ content: 'No active Sudoku game found for you!', flags: MessageFlags.Ephemeral });
    }
    ...
    const gridNum = gameState.selectedGrid;
    gameState.selectedCell = null;
    gameState.selectedValue = null;
    gameState.conflictSet = getConflictingCells(gameState.puzzle);

    const buffer = await drawSudokuGrid(gameState.puzzle, gameState.prefilledSet, gameState.theme, null, null, gridNum, gameState.conflictSet, gameState.notes.map(s => Array.from(s)));
    ...
    const buttonRows = createCellButtons(gameState.theme, new Set(gameState.prefilledSet), gameState.puzzle, gridNum, gameState.mode, gameState.joinedPlayers);
    ...
    if (filledCells === 81) {
        ...
        if (isCorrect) {
            ...
            container.addSeparatorComponents(s => s);
            container.addTextDisplayComponents(td => td.setContent('✖️ The puzzle is not correct yet. Keep trying!'));
        }
    }

    container.addSeparatorComponents(s => s).addActionRowComponents(...buttonRows.filter(Boolean));
    games.set(gameId, gameState);
    await SudokuGame.updateOne( { gameId }, { puzzle: gameState.puzzle, selectedGrid: gameState.selectedGrid, selectedCell: gameState.selectedCell, selectedValue: gameState.selectedValue, conflictSet: [...gameState.conflictSet], lastUpdated: Date.now(), });
