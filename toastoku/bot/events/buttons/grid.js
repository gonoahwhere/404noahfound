...
const { titleCase, getTurnEmbedFields, createCellButtons, drawSudokuGrid, isPuzzleInSolutions } = require('../../commands/global/sudoku');
...
module.exports = async function handleGrid(interaction) {
    if (!interaction.customId.startsWith('grid_')) return;
    ...
    const gridNum = parseInt(interaction.customId.split('_')[1]) - 1;
    gameState.selectedGrid = gridNum;
    ...
    await SudokuGame.updateOne({ gameId }, { puzzle: gameState.puzzle, selectedGrid: gridNum, selectedCell: null, selectedValue: null, conflictSet: [...gameState.conflictSet], lastUpdated: Date.now(), });
